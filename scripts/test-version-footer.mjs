...
try {
  ...
  await page.evaluate(() => localStorage.setItem('radiant.server', JSON.stringify({ base: 'http://localhost:5964' })))
  await page.reload({ waitUntil: 'networkidle' })
  const remoteFooter = page.locator('.sidebar-version')
  await remoteFooter.waitFor()
  assert.equal(await remoteFooter.innerText(), `localhost:5964 · Allegretto ${pkg.version}`)
  assert.match(await remoteFooter.getAttribute('title'), new RegExp(`Radiant engine ${pkg.radiantEngineVersion}`))
