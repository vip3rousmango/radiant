...
const GUIDE = [
  {
    ...
    items: [
      ...
      ['The footer shows both version identities', 'The bottom-left footer names the Allegretto build you are running. Hover it to see the Radiant engine version underneath, which helps separate an agency app update from an upstream engine update.'],
      ['A stale server cannot change the displayed build', 'The footer and Settings \u2192 About now read the Allegretto version from the page itself, not from the connected server. If an old packaged server is still listening on the development port, the app can still tell you its own version and show the Radiant engine version separately.'],
