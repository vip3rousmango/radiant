...
const GUIDE = [
  {
    ...
    items: [
      ...
      ['The footer shows both version identities', 'The bottom-left footer names the Allegretto build you are running. Hover or keyboard-focus it to see the Radiant engine version underneath. When this window uses another Mac, it says connected to that Mac and may add that server\u2019s version as a diagnostic, while keeping the local build name truthful.'],
      ['A stale server cannot change the displayed build', 'The footer and Settings \u2192 About read the Allegretto version from the page itself, not from the connected server. If an old packaged server is still listening on the development port, the app can still tell you its own version and show the Radiant engine version separately.'],
  ...
  },
  {
    ...
    items: [
      ...
      ['Removing a phone model leaves the Models screen usable', 'After you remove the current model, the phone keeps that detail view steady until the closing motion finishes instead of changing into the model picker, then Home stops offering weights that are no longer installed. Your conversations stay on the phone.'],
